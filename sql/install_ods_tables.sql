...
BEGIN
  ...
  EXECUTE IMMEDIATE '
    CREATE TABLE EQUITY.ODS_PRICE_OHLCV (
        ...
        "RECORD_DTS"      VARCHAR2(4000 BYTE),
        "BATCH_ID"        VARCHAR2(50)   NOT NULL, 
        "LOAD_TIME"       VARCHAR2(50)   NOT NULL,
        "RECORD_DTS_DB"    TIMESTAMP WITH LOCAL TIME ZONE DEFAULT CURRENT_TIMESTAMP
    )';
...
-- 索引创建
BEGIN
  EXECUTE IMMEDIATE 'CREATE INDEX EQUITY.IDX_ODS_YAHOO_BATCH ON EQUITY.ODS_PRICE_OHLCV("BATCH_ID")';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE = -955 THEN 
      DBMS_OUTPUT.PUT_LINE('Index IDX_ODS_YAHOO_BATCH already exists, skipping.');
    ELSE
      RAISE;
    END IF;
END;
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE INDEX EQUITY.IDX_ODS_YAHOO_CODE ON EQUITY.ODS_PRICE_OHLCV("CODE")';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE = -955 THEN 
      DBMS_OUTPUT.PUT_LINE('Index IDX_ODS_YAHOO_CODE already exists, skipping.');
    ELSE
      RAISE;
    END IF;
END;
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE INDEX EQUITY.IDX_ODS_YAHOO_TS ON EQUITY.ODS_PRICE_OHLCV("RAW_TIMESTAMP")';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE = -955 THEN 
      DBMS_OUTPUT.PUT_LINE('Index IDX_ODS_YAHOO_TS already exists, skipping.');
    ELSE
      RAISE;
    END IF;
END;
